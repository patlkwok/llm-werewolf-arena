...
The browser suite builds and runs the production app with Playwright's Chromium on Windows, macOS, and Linux. On Linux, Playwright may also require OS browser dependencies; see its installation instructions. The unit/integration suite uses deterministic fake models and makes no live OpenRouter calls. Automated checks verify application behavior and rule correctness; large-scale live-model balance testing is not a project goal.
